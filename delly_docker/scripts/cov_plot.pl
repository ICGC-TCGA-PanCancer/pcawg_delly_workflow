...
if ($scale ==0 ) {
 	print STDERR "SCALE = 0; exiting\n";
	exit;
}
$scale = 1 / $scale unless ($scale == 0);
...
open (IN, "/usr/bin/read_depth_ratio_plusCovSigSeg.R") or die "Cannot read read_depth_ratio_plusCovSigSeg.R from /usr/bin/";
while (<IN>) {
  if (/^LIBRARY/) {
	  print OUT "if(\"DNAcopy\" %in% rownames(installed.packages()) == FALSE) {install.packages(\"/usr/bin/DNAcopy_1.38.1.tar.gz\")}\nlibrary(\"DNAcopy\")\n";
