...
# multisample?
n_tum_names = length(sampleNames) -1
n_tum = ncol(cov1_full) -3
tum_names = sampleNames
if (n_tum > 1){
	cat(n_tum, "multi tumor samples found!")
	if (n_tum_names < n_tum){
		tum_names = list()
		for (i in 1:n_tum){
		tum_names[[i]] = paste(sampleNames[1], "_0", i, sep="")
		}
		tum_names = unlist(tum_names)
		
	}
} 
covcolnames = names(cov1_full[c(1:4)])
for (tum in 1:n_tum){
	colnum = tum + 3
	cov1_sub = cov1_full[,c(1:3,colnum)]
	names(cov1_sub) = covcolnames
	sampleNamesSub = c(tum_names[tum], sampleNames[2])
...
cbind(cov1_sub, LRR=log2(cov1_sub$V4/cov2_full$V4))->cov_combi
cov_combi[cov_combi=="NaN" | cov_combi=="-Inf" | cov_combi=="Inf"]<-0
CNA.object <- CNA(cov_combi[,5],
                  cov_combi[,1],
                  cov_combi[,2],
                  data.type="logratio",sampleid=sampleNamesSub[1])
...
for(myChr in chr) {
	...
	# Read coverage files
	cov1 = cov1_sub[cov1_sub[,1]==myChr,]
